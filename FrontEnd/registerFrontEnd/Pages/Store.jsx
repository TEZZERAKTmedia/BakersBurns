...
import '../PagesCss/store.css'; // Import the CSS file
...
const Store = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await registerApi.get('/store'); // Fetch products from store
  ...
  const handleAddToCart = async (productId) => {
    ...
    try {
      const response = await registerApi.post('/cart/add-to-cart', { userId, productId, quantity: 1 }, {
        headers: {
